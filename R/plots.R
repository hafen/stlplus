...
plot.stlplus <- function(
  ...
  if (x$pars$fc.number == 0) {
    ...
    if (any(is.na(getraw(x)))) {
      ...
      d$values[
        d$ind == "seasonal" & d$which == "isxNA" & !is.na(getraw(x))
      ] <- NA
    ...
    if (any(is.na(getraw(x)))) {
      ...
      for (i in 1:fc.number) {
        d$values[
          d$ind == fc.name[i] & d$which == "isxNA" & !is.na(getraw(x))
        ] <- NA
      }
    }
  }

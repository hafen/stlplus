...
stlplus.default <- function(
  ...
  if (missing(n.p)) {
    stop(
      "must specify periodicity of seasonal (either explicitly or through a time series object)"
    )
  }
  n.p <- as.integer(n.p)
  ...
  wincheck <- function(x) {
    x <- nextodd(x)
    if (any(x <= 0)) {
      stop("Window lengths must be positive.")
    }
    x
  }
  ...
  get.t.window <- function(t.dg, s.dg, n.s, n.p, omega) {
    ...
    betac1 <- coefs_b$a[s.dg] +
      coefs_b$b[s.dg] * omega +
      coefs_b$c[s.dg] * omega^2
    betac2 <- coefs_c$a[s.dg] +
      coefs_c$b[s.dg] * omega +
      coefs_c$c[s.dg] * omega^2
    f_c <- (1 - (betac0 + betac1 / n.s + betac2 / n.s^2)) / n.p

    # choose
    betat0 <- coefs_a$a[t.dg] + coefs_a$b[t.dg] * omega
    betat1 <- coefs_b$a[t.dg] +
      coefs_b$b[t.dg] * omega +
      coefs_b$c[t.dg] * omega^2
    betat2 <- coefs_c$a[t.dg] +
      coefs_c$b[t.dg] * omega +
      coefs_c$c[t.dg] * omega^2

    betat00 <- betat0 - f_c

    n.t <- nextodd(
      (-betat1 - sqrt(betat1^2 - 4 * betat00 * betat2)) / (2 * betat00)
    )
  ...
  if (is.null(sub.labels)) {
    ...
    if (length(sub.labels) != n.p) {
      stop("sub.labels must be of length n.p")
    }
  ...
  if (is.null(s.jump) || length(s.jump) == 0) {
    s.jump <- ceiling(s.window / 10)
  }
  if (is.null(t.jump) || length(t.jump) == 0) {
    t.jump <- ceiling(t.window / 10)
  }
  if (is.null(l.jump) || length(l.jump) == 0) {
    l.jump <- ceiling(l.window / 10)
  }
